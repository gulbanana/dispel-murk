...
public class Build
{
    ...
    {
        var input = "foo";
        var r = LineParser.AttributedText(input);
    ...
    public void SimpleMessage()
    {
        ...
        Assert.Equal("player1", ast.Username);
        Assert.Equal("00:01", ast.Timestamp);
        Assert.Equal("foo", ast.Message.Flatten());
    }
    ...
    public void ComplexMessage()
    {
        ...
        var ast = r.Tree.Build<Line>();
        Assert.Equal(3, ast.Message.Runs.Length);

        var r0 = ast.Message.Runs[0];
